...
def parse_listing(html):
    ...
    d = next_data(html)
    if not d: return [], 0
    s = d.get("props", {}).get("initialReduxState", {}).get("search", {})
    byId = s.get("byId", {}) or {}
    order = s.get("adsIds") or list(byId.keys())
    out = []
    for adId in order:
        ad = byId.get(str(adId)) or byId.get(adId)
        if not ad: continue
        uid = ad.get("userId")
        if uid in (None, "", 0): continue
        out.append((str(uid), ad.get("adUrl") or "",
                    ad.get("groupName") or ad.get("categoryName") or ""))
    return out, int(s.get("pages") or 0)
